...
export class VideoRTC extends HTMLElement {
	...
	set src(value) {
		...
		if (value.startsWith('http')) {
			...
			// Use current origin and replace http/https with ws/wss
			value = location.origin.replace(/^http/, 'ws') + value;
		}
	...
	onwebrtc() {
		...
		pc.addEventListener('icecandidate', (ev) => {
			if (ev.candidate && this.mode.indexOf('webrtc/tcp') >= 0 && ev.candidate.protocol === 'udp')
				return;
